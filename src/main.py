...
def list_workflows():
    return {"workflows": [json.loads(r['payload']) | {'id':r['id'],'created_at':r['created_at']} for r in rows('workflow')]}
...
@app.get('/api/runs')
def list_runs():
    return {"runs": [json.loads(r['payload']) | {'id':r['id'],'created_at':r['created_at']} for r in rows('run')]}
