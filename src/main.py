...
class WorkflowStep(BaseModel):
    ...
    # NOTE: default_factory (never a shared mutable default — `params: dict = {}`
    # would be ONE dict object shared by every step instance).
...
BUILTIN_TOOLS = {
    "search": lambda p: f"Searched: {p.get('query', 'n/a')} -> returned results",
    "parse": lambda p: f"Parsed content of length {p.get('length', 0)}",
    "summarize": lambda p: (
        f"Summarized text with target length {p.get('target_length', 'brief')}"
    ),
    "export": lambda p: f"Exported to {p.get('format', 'csv')}",
}


def _sanitize_name(name: str) -> str:
    return re.sub(r"[^a-z0-9_-]", "", name.replace(" ", "_").lower()) or "workflow"


@app.post("/api/workflows")
def create_workflow(wf: Workflow):
    safe_name = _sanitize_name(wf.name)
    # Collision guard: "a/b" and "a b" both sanitize to "a_b" — refuse the
    # second instead of silently overwriting a saved workflow file.
    path = WORKFLOWS_DIR / f"{safe_name}.json"
    if path.exists():
        existing = json.loads(path.read_text())
        if existing.get("name") != wf.name:
            raise HTTPException(
                status_code=409,
                detail=f"Workflow name collides with an existing saved file ({safe_name}.json). "
                "Choose a name that sanitizes differently.",
            )
...
def run_workflow(name: str):
    items = [
        r for r in rows("workflow") if json.loads(r["payload"]).get("name") == name
    ]
    if not items:
    ...
    save_record("run", name, json.dumps(record))
    return record
